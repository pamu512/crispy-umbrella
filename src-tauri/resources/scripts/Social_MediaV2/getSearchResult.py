...
PLATFORM_URLS = {
    ...
    "youtube": "https://www.social-searcher.com/youtubecse.html",
    "reddit": "https://www.social-searcher.com/redditcse.html",
    "snapchat": "https://www.social-searcher.com/snapchatcse.html"
}
...
def get_base_params(platform, updated_params=None):
    ...
    if updated_params:
        if isinstance(updated_params, dict):
            ...
            platform_keys = ["facebook", "twitter", "instagram", "linkedin", "tiktok", "pinterest", "youtube", "reddit", "snapchat"]
...
def fetch_google_cse(query, base_params, num=100, platform_name="", start_time=None, end_time=None):
    global rt  # Declare global rt at the beginning of the function
    url = "https://cse.google.com/cse/element/v1"

    all_results = []
    # If filtering is enabled, we might need more pages, but we are limited by API (100 results max usually)
    # We'll stick to fetching enough pages to cover 'num' potential results, 
    # but since we can't deep page easily past 100, we just filter what we get.
    ...
    while i < max_pages:
        ...
        while page_retry_count <= max_page_retries:
            ...
            if not match:
                ...
                # For TikTok/Pinterest/YouTube/Reddit/Snapchat, check if response contains error or empty results
                if platform_name.upper() in ["TIKTOK", "PINTEREST", "YOUTUBE", "REDDIT", "SNAPCHAT"]:
            ...
            try:
                ...
                # Filter results by date if start_time or end_time is provided
                    ...
                    try:
                        ...
                        if end_time:
                            ...
                            if end_dt:
                                ...
                                print(f"[WARNING] Could not parse end_time: {end_time}")
                    except Exception as e:
                        print(f"[WARNING] Error parsing date filters: {e}")
                    
                    if start_dt or end_dt:
                        for r in results:
                            original_abstract = r.get("contentNoFormatting", "")
                            # Use extract_date_from_abstract to get date string
                            # Note: extract_date_from_abstract is defined later but available at runtime
                            date_str, _ = extract_date_from_abstract(original_abstract)
                            
                            try:
                                # date_str is "%Y-%m-%d %H:%M:%S"
                                item_dt = datetime.strptime(date_str, "%Y-%m-%d %H:%M:%S")
                        ...
                        if len(results) != len(filtered_results):
                            print(f"[FILTER] {platform_name} page {i+1}: {len(results)} -> {len(filtered_results)} results after date filtering")
                ...
                # Debug for TikTok/Pinterest/YouTube/Reddit/Snapchat
                if platform_name.upper() in ["TIKTOK", "PINTEREST", "YOUTUBE", "REDDIT", "SNAPCHAT"] and len(results) == 0:
...
def convert_relative_time_to_datetime(time_str):
    ...
    # Persian (Jalali) calendar format: "Bahman 14, 1404" or "Bahman 14, 1404 AP"
    persian_month_map = {
        'farvardin': 1, 'ordibehesht': 2, 'khordad': 3, 'tir': 4, 'mordad': 5,
        'shahrivar': 6, 'mehr': 7, 'aban': 8, 'azar': 9, 'dey': 10,
        'bahman': 11, 'esfand': 12
    }
    persian_match = re.search(
        r'(farvardin|ordibehesht|khordad|tir|mordad|shahrivar|mehr|aban|azar|dey|bahman|esfand)\s+(\d{1,2})[,\s]+(\d{4})',
        time_str, re.IGNORECASE
    )
    if persian_match:
        month_name, day_str, year_str = persian_match.groups()
        try:
            jmonth = persian_month_map.get(month_name.lower())
            jday = int(day_str)
            jyear = int(year_str)
            if jmonth and 1 <= jday <= 31 and jyear > 0:
                try:
                    import jdatetime
                    j_date = jdatetime.date(jyear, jmonth, jday)
                    g_date = j_date.togregorian()
                    return g_date.strftime("%Y-%m-%d 00:00:00")
                except ImportError:
                    if not hasattr(convert_relative_time_to_datetime, "_jdatetime_warned"):
                        print("[DATE] Persian date detected but jdatetime not installed. Run: pip install jdatetime")
                        convert_relative_time_to_datetime._jdatetime_warned = True
        except (ValueError, KeyError):
            pass
...
def extract_date_from_abstract(abstract, platform=None):
    """
    Extract date information from abstract text (first 3 words) and convert to datetime
    
    Args:
        abstract (str): The abstract content
        platform (str): Optional platform name for debug print (e.g. "facebook")
...
def _query_to_folder_name(query):
    """
    Convert query to filesystem-safe folder name for output path.
    Strips wrapping " so "Three Ireland" -> Three Ireland (playwright/docker can find it).
    """
    if not query:
        return query
    q = query.strip()
...
def save_to_csv(results, keyword, platform, filename=None, output_path=None):
    ...
    with open(filepath, "w", newline="", encoding="utf-8-sig") as f:
        ...
        for r in results:
            original_abstract = r.get("contentNoFormatting", "")
            date_info, cleaned_abstract = extract_date_from_abstract(original_abstract, platform=platform)
            # Decode URL (e.g. %3F -> ?, %3D -> =) for readable CSV format
            url = unquote(r.get("url", ""))
            writer.writerow([
...
def search_all_platforms(query, total_num=100, updated_params=None, output_path=None, start_time=None, end_time=None):
    """
    ...
    Args:
        ...
        updated_params (dict): Updated parameters
        output_path (str): Optional custom output directory path
        start_time (str): Start date filter (YYYY-MM-DD)
        end_time (str): End date filter (YYYY-MM-DD)
    """
    global rt  # Declare global rt at the beginning
    results = {}
    
    platforms = ["facebook", "twitter", "instagram", "linkedin", "tiktok", "pinterest", "youtube", "reddit", "snapchat"]
    
    # Use path-safe folder name for output (playwright/docker expect no " in path)
    folder_name = _query_to_folder_name(query)
    
    print(f"Starting search on {len(platforms)} platforms for '{query}'...")
    print(f"Target results per platform: {total_num}")
    if start_time or end_time:
        print(f"Date filter: {start_time or 'Any'} to {end_time or 'Any'}")
    print("-" * 50)
    
    # Create output directory structure upfront (use folder_name for path compatibility)
    if output_path:
        base_output_dir = Path(output_path) / folder_name
    else:
        base_output_dir = Path("output") / folder_name
    
    try:
        base_output_dir.mkdir(parents=True, exist_ok=True)
        print(f"[INFO] Ensured output directory exists: {base_output_dir}")
    except Exception as e:
    ...
    for platform in platforms:
        ...
        while platform_retry_count <= max_platform_retries:
            try:
                ...
                platform_results = fetch_google_cse(query, base_params, num=total_num, platform_name=platform.upper(), start_time=start_time, end_time=end_time)
                
                # Solution 1: Check if results are less than expected, restart Docker and retry
                # Note: If filtering is active, having fewer results is expected, so we relax this check
                expected_min = total_num if not (start_time or end_time) else 0
                
                if len(platform_results) < expected_min and platform_retry_count < max_platform_retries:
                ...
                # Save to CSV (using folder_name for path compatibility with playwright/docker)
                filepath = save_to_csv(platform_results, folder_name, platform, output_path=output_path)
                ...
                # If this is snapchat (last platform), stop containers
                if platform == "snapchat":
                    print(f"[SHUTDOWN] Snapchat search completed, stopping Docker containers...")
                ...
                if platform_retry_count < max_platform_retries:
                    ...
                    if restart_docker_tor():
                        ...
                        results[platform] = []
                        # If this is snapchat and we're giving up, stop containers
                        if platform == "snapchat":
                            print(f"[SHUTDOWN] Snapchat search failed, stopping Docker containers...")
                            stop_docker_containers()
                        break
                else:
                    # Max retries reached
                    ...
                    # If this is snapchat and we're giving up, stop containers
                    if platform == "snapchat":
                        print(f"[SHUTDOWN] Snapchat search failed after retries, stopping Docker containers...")
                        stop_docker_containers()
                    break
