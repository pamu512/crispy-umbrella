...
def normalize_query(query):
    """
    Normalize query for URL/search: when query has spaces (multi-word),
    wrap with " and keep spaces (requests will encode to %22 and +).
    Use raw chars, NOT pre-encoded, to avoid double encoding.
    """
    if " " in query:
        inner = query.replace('"', '').strip()
        return '"' + inner + '"'
    return query
...
class SocialMediaSearchOrchestrator:
    ...
    def execute_search(self, query, total_num=100, updated_params=None, output_path=None, start_time=None, end_time=None):
        """Execute search"""
        ...
        try:
            ...
            results = search_all_platforms(query, total_num, updated_params, output_path=output_path, start_time=start_time, end_time=end_time)
    ...
    def run_complete_search(self, query, total_num=100, use_updated_params=True, output_path=None, start_time=None, end_time=None):
        """Run complete search process"""
        # Normalize query: multi-word -> spaces to +, wrap with %22
        ...
        if start_time or end_time:
            self.logger.info(f"[FILTER] Date filter: {start_time or 'Any'} to {end_time or 'Any'}")
        ...
        try:
            ...
            results = self.execute_search(query, total_num, updated_params, output_path=output_path, start_time=start_time, end_time=end_time)
...
def main():
    ...
    parser.add_argument('--start-time', type=str, help='Start date filter (YYYY-MM-DD)', default=None)
    parser.add_argument('--end-time', type=str, help='End date filter (YYYY-MM-DD)', default=None)
    
    # Allow unknown args to avoid crashing if extra args are passed (though we prefer strict parsing usually)
    args, unknown = parser.parse_known_args()
    if unknown:
        print(f"[WARNING] Unknown arguments ignored: {unknown}")

    # Clean up arguments that might have surrounding quotes due to shell passing issues
    if args.target:
        args.target = args.target.strip('"\'')
    if args.output:
        args.output = args.output.strip('"\'')
    if args.start_time:
        args.start_time = args.start_time.strip('"\'')
    if args.end_time:
        args.end_time = args.end_time.strip('"\'')
    ...
    if args.target is None:
        ...
        print("3. Execute search on all social media platforms (Facebook, Twitter, Instagram, LinkedIn, TikTok, Pinterest, YouTube, Reddit, Snapchat)")
        ...
        start_time = input("Start date (YYYY-MM-DD) [Optional]: ").strip() or None
        end_time = input("End date (YYYY-MM-DD) [Optional]: ").strip() or None
    ...
    orchestrator = SocialMediaSearchOrchestrator()
    orchestrator.run_complete_search(query, total_num, use_updated_params, output_path=output_path, start_time=start_time, end_time=end_time)
